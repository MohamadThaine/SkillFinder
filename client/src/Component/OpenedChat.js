...
const OpenedChat = ({ chat, pic, setOpenedChat }) => {
    ...
    return (
        <>
            ...
            <div className='message-tools border-top'>
                <textarea placeholder='Type a message...' className='message-input' value={message} onChange={e => setMessage(e.target.value)} />
...
const Message = ({ message, loggedUser }) => {
    ...
    return (
        <div className={`d-flex align-items-center ${message.Sender_ID === loggedUser.id ? 'right-message' : ''}`}>
            ...
            <div className={`message bubble ${message.Sender_ID === loggedUser.id ? 'sent-msg' : 'recived-msg'}`}>
